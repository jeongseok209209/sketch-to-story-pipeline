...
EXAONE GGUF uses llama.cpp GPU offload by default when an NVIDIA GPU is detected.
...
If VRAM is too small, lower the value. To force EXAONE GGUF through CPU:

```powershell
$env:LLAMA_GPU_LAYERS="0"
```

BLIP, OpenCLIP, Qwen, GPT-2, and NLLB use PyTorch CUDA only when the installed
PyTorch build has CUDA support. To install the CUDA 12.4 PyTorch build manually:
...
To prevent automatic CUDA PyTorch install attempts during a run:
