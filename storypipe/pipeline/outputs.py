"""[담당 3 · 파이프라인] 이야기 선택 + 결과/요약 파일 작성 (CLI·doctor 공용 지원 모듈).

이야기 폴더 해석, 실험 A 결과 정규화, 표준 스토리 파일(JSON/TXT/HTML), 실행 요약/실패 기록,
블라인드 평가 매핑 리셋을 담당한다. common만 의존하므로 cli/doctor에서 순환 없이 가져다 쓴다.
"""
...
from storypipe.common.io import write_json as _write_json
