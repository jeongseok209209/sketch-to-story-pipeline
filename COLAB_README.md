...
SSH 인증이 없는 Colab에서는 GitHub의 HTTPS clone 주소를 사용하세요. 노트북에서 `ERROR: Could not open requirements file: [Errno 2] No such file or directory: 'requirements-colab.txt'`가 나오면 저장소 루트가 아닌 `/content` 같은 다른 폴더에서 설치 셀을 실행한 것입니다. `%cd sketch-to-story-pipeline`을 실행하거나, 최신 노트북의 의존성 설치 셀을 다시 실행하면 저장소 폴더를 자동으로 찾아 이동합니다. 이어서 `FileNotFoundError: ... '/content/inputs'`가 나오면 이야기 선택 셀이 저장소 루트를 못 찾은 상태이므로, 최신 노트북으로 다시 열고 의존성 설치 셀부터 순서대로 실행하세요.
